...
$model = [
    ...
    'write_data' => function ($data) {
        // Utiliser Enums\DATA_PATH au lieu de __DIR__
        ...
        if (file_put_contents(Enums\DATA_PATH, $json_data) === false) {
            error_log("Erreur d'écriture dans le fichier : " . Enums\DATA_PATH);
            return false;
        }
        
        // Donner les bonnes permissions au fichier
        chmod(Enums\DATA_PATH, 0664);
    ...
    'add_promotion_to_db' => function ($promotion) {
        try {
            ...
            // Générer un nouvel ID
            $max_id = 0;
            foreach ($data['promotions'] as $existing_promotion) {
                $max_id = max($max_id, $existing_promotion['id']);
            }
            $promotion['id'] = $max_id + 1;
            ...
            // Sauvegarder les données
            file_put_contents(
                dirname(__DIR__) . '/data/data.json', 
    ...
    $active_promotions = count(array_filter($data['promotions'] ?? [], function ($promotion) {
        return $promotion['status'] === Status::ACTIVE->value; // Utilisation de l'énumération Status
    }));
    ...
    'get_statistics' => function () use (&$model) {
        ...
        // Compter les apprenants actifs (uniquement ceux dans les promotions actives)
        $active_learners = 0;
        foreach ($active_promotions as $promotion) {
            $active_learners += count($promotion['apprenants'] ?? []);
        }
        
        // Compter les référentiels utilisés dans les promotions actives
        $active_referentials = [];
        foreach ($active_promotions as $promotion) {
            foreach ($promotion['referentiels'] ?? [] as $referentiel) {
                $active_referentials[$referentiel] = true;
            }
        }
        
        return [
            'active_learners' => $active_learners,
            'total_referentials' => count($active_referentials),
